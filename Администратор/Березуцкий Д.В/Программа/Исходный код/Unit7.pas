...
begin
  ...
  begin
    Close;
    sql.clear;
    sql.Add('UPDATE domalg SET Задание = '+#39+s+#39+' WHERE Код = '+IntToStr(id));
    ExecSQL;
  end;
  with ADOQuery1 do
  begin
    Close;
    sql.Clear;
    sql.Add('SELECT * FROM domalg WHERE Класс = '+Edit1.Text+' AND Буква = '+#39+Edit2.Text+#39+' AND Предмет = '+IntToStr(ComboBox1.ItemIndex)+' AND Времявыполнения > '+IntToStr(DateTimeToUnix(Date)));
    Open;
  end;
end;
...
begin
  ...
  begin
    ...
    BitBtn4.Caption:= 'Скрыть задания';
  ...
  begin
    ...
    Memo2.Visible:= false;
    BitBtn4.Caption:= 'Показать задания';
  end;
end;
...
begin
  ...
  begin
    Close;
    sql.Clear;
    sql.Add('SELECT * FROM Dostup WHERE Логин = '''+Form1.Edit11.Text+''' AND Класс = '+Edit1.Text+' AND Буква = '''+Edit2.Text+''' AND Предмет = '+IntToStr(ComboBox1.ItemIndex));
    Open;
  end;
  ...
  begin
    Close;
    sql.clear;
    sql.Add('INSERT INTO domalg(Задание, Класс, Буква, Датавыполнения, Предмет, Времявыполнения) VALUES('+#39+s+#39+', '+#39+Edit1.Text+#39+', '+#39+Edit2.Text+#39+', '+#39+DateTimeToStr(DateTimePicker1.Date)+#39+', '+#39+IntToStr(ComboBox1.ItemIndex)+#39+', '+#39+IntToStr(utime)+#39+')');
    ExecSQL;
  end;
...
begin
  ...
  begin
    ...
    begin
      Close;
      sql.Clear;
      sql.Add('DELETE FROM domalg WHERE Код = '+IntToStr(id));
      ExecSQL;
    end;
    with ADOQuery1 do
    begin
...
begin
  if Button2.Caption = 'Показать просроченные задания' then
  begin
    with ADOQuery1 do
    begin
      Close;
    ...
    Button2.Caption:= 'Показать активные задания';
  end
  else
  begin
    with ADOQuery1 do
    ...
    Button2.Caption:= 'Показать просроченные задания';
  end;
end;
